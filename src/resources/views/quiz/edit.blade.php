...
<x-app-layout>
    <div class="bg-white min-h-screen flex flex-col items-center justify-center text-white">
        <div class="imp_margin_04 max-w-xl w-full p-6 md:p-8 bg-gray-500 rounded-lg shadow-lg shadow-gray-400">
            <h1 class="text-2xl font-bold mb-5 text-center">@lang('messages.editQuiz')</h1>
            <form id="main-form" method="POST" action="{{ route('quiz.update', $quiz) }}"
                ...
                <div class="mb-4">
                    ...
                    <input type="text"
                        ...
                        id="quiz" name="quiz" placeholder="@lang('messages.enterQuiz')" value="{{ $quiz->title }}">
                </div>
                @csrf
                <div class="mb-4">
                            ...
                            @foreach ($users as $user)
                                @if ($quiz->owner_id == $user->id)
                                    <option value="{{ $user->name }}"
                                        @if ($quiz->owner_id == $user->id) selected @endif>{{ $user->name }}</option>
                                @else
